...
def_coords VARCHAR(100),
atk_coords VARCHAR(100),
team_capacity INT,
created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
)
`;
const queryZonesCiudad = `
...
id INT AUTO_INCREMENT PRIMARY KEY,
name VARCHAR(100) UNIQUE,
coords_team1 VARCHAR(100),
coords_team2 VARCHAR(100),
created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
)
`;
const queryRanking = `
...
await pool.query(queryBlacklist);
await pool.query(querySedes);
await pool.query(queryZonesCiudad);
await pool.query(queryZonesCayo);
await pool.query(queryDuels);
await pool.query(queryRanking);
console.log('✅ BASE DE DATOS: Tablas verificadas/creadas (Usuarios + Blacklist + Sedes + Zonas BR + Duelos + Ranking).');
} catch (error) {
...
await pool.execute(
`INSERT IGNORE INTO users (discord_id, role, username, server_nickname, join_date) VALUES (?, ?, ?, ?, NOW())`,
...
{
name: "Barragem", coords: "1359.65, 1139.28, 113.76, 76.89",
def_coords: "1379.13, 1174.25, 114.22, 45.35",
atk_coords: "897.87, 17.01, 78.88, 158.00",
cap_rec: "25vs25"
},
{
name: "Warlocks", coords: "-1895.63, 2026.71, 140.73, 159.60",
...
{
name: "La04", coords: "-1573.41, 95.68, 58.43, 133.83",
...
{
name: "Sindicato", coords: "-1550.37, 879.80, 181.32, 35.04",
def_coords: "-1551.66, 858.45, 181.94, 272.42",
atk_coords: "-1806.40, 454.49, 128.30, 40.63",
cap_rec: "20vs20"
},
{
name: "Anonimato", coords: "-126.41, 1003.46, 235.73, 338.78",
...
{
name: "Blinders", coords: "-3430.12, 544.17, 10.79, 183.22",
...
await pool.execute(`
INSERT INTO sedes (name, coords, is_raidable, def_coords, atk_coords, team_capacity)
VALUES (?, ?, ?, ?, ?, ?)
ON DUPLICATE KEY UPDATE
coords=VALUES(coords), is_raidable=VALUES(is_raidable),
def_coords=VALUES(def_coords), atk_coords=VALUES(atk_coords),
team_capacity=VALUES(team_capacity)
`, [
sede.name,
sede.coords,
isRaidable,
sede.def_coords || null,
sede.atk_coords || null,
capacity
]);
}
console.log("✅ BASE DE DATOS: Sedes migradas/verificadas.");
} catch (e) {
...
VALUES (?, ?)
ON DUPLICATE KEY UPDATE coords=VALUES(coords)
`, [name, coords]);
}
};
await seedList(rawCiudad, 'br_zones_ciudad');
await seedList(rawCayo, 'br_zones_cayo');
console.log("✅ BASE DE DATOS: Zonas BR migradas separadas (Ciudad / Cayo).");
} catch (e) {
